...
const academicTypes = new Set(["university", "research_institute", "think_tank"]);

export default function AcademicInstitutionsPage() {
  const academicInstitutions = institutions.filter((institution) => academicTypes.has(institution.type));

  return (
    <div className="space-y-8">
      <div>
        <p className="text-xs uppercase tracking-[0.3em] text-(--text-secondary)">Academic partners</p>
        <h1 className="text-4xl font-semibold text-foreground">Universities & research hubs</h1>
        <p className="text-(--text-secondary)">Academic institutions leading research briefings, speaker series, and thematic cohorts.</p>
      </div>
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        ...
        {academicInstitutions.length === 0 && (
          <div className="rounded-4xl border border-(--border-subtle) bg-surface p-6 text-(--text-secondary)">
            No academic institutions available yet.
          </div>
        )}
      </div>
    </div>
